    document.addEventListener('DOMContentLoaded', () => {
        ...
        const validators = {
            username: (value) => {
                ...
                if (value.includes(' ')) {
                    elements.usernameError.textContent = 'Tên đăng nhập không được chứa khoảng trắng';
                    return false;
                }
                if (!/^[a-zA-Z0-9_]+$/.test(value)) {
                    elements.usernameError.textContent = 'Tên đăng nhập chỉ được chứa chữ cái, số và dấu gạch dưới';
                    return false;
                }
                return true;
            },
        ...
        // Show server message (error or success)
        const showServerMessage = (message, isSuccess = false) => {
            elements.serverError.textContent = message;
            elements.serverError.classList.remove('error', 'success');
            elements.serverError.classList.add(isSuccess ? 'success' : 'error');
        ...
        const handleAuth = async () => {
            ...
            try {
                ...
                showServerMessage(result.message, true);
